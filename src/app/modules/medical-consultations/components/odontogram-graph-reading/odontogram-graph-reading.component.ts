...
@Component({
	selector: 'app-odontogram-graph-reading',
	standalone: true,
	imports: [NgStyle],
	templateUrl: './odontogram-graph-reading.component.html',
	styleUrl: './odontogram-graph-reading.component.scss',
})
export class OdontogramGraphReadingComponent {
	...
	private addColor(piezas: any[]) {
		...
		const formattedData = this.teeth().map((tooth) => {
			...
			return {
				...tooth,
				color: pieza && pieza.faseodontogramaid === 2 ? '#aac9e8' : '#fff',
			};
		});
